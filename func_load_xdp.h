...
static const struct option_wrapper long_options[] = {
	...
	{{"unload",      no_argument,		NULL, 'U' },
	 "Отсоединить программу XDP от интерфейса."},
...
struct bpf_object *load_bpf_object_file(const char *filename) {
	...
	if (err) {
		fprintf(stderr, "ОШИБКА: не могу получить информацию о программе - %s\n",
			strerror(errno));
		return NULL;
	}
...
static int xdp_link_detach(int ifindex, __u32 xdp_flags) {
	...
	if ((err = bpf_set_link_xdp_fd(ifindex, -1, xdp_flags)) < 0) {
		...
		printf("Программа XDP успешно отсоеденина от интерфейса!\n");
	}
	return EXIT_OK;
}
...
int xdp_link_attach(int ifindex, __u32 xdp_flags, int prog_fd) {
	...
	if (err < 0) {
		...
		case EBUSY:
		case EEXIST:
			fprintf(stderr, "Подсказка: программа XDP уже загружена,"
